	...
	public class DefaultEngagementValueProvider : IEngagementValueProvider {
		...
		public Dictionary<string, List<IEngagementValue>> RelevantValues {
			get {
				...
				Dictionary<string, IEngagementValue> vals = ConfigUtil.Context.EVProvider.Values.SelectMany(a => a.Value).OrderByDescending(a => a.LastUpdated).Take(clickCountLimit).ToDictionary(a => a.LastUpdated.ToString(DateTimeFormat));
